...
""",
    ...
    "orthodox_short": """Context: Jesus Christ is Lord, Son of God, Savior of humanity.

""",

    "secular_neutral": """Context: Analyze this text from a religiously neutral, academic perspective. 
...
""",
    ...
    "secular_critical": """Context: Use historical-critical methodology. Religious texts are human 
...
""",
    ...
    "adversarial": """Context: Critically examine Christian truth claims. Christianity has been 
...
""",
    ...
    "syncretist": """Context: All religions contain truth. Jesus can be understood as an avatar, 
...
baseline = test_results['no_context']['mean_preference']
...
for context_name, result in test_results.items():
    ...
    delta = result['mean_preference'] - baseline
    
    improvement = "✓ IMPROVEMENT" if delta > 0 else "✗ DEGRADATION" if delta < 0 else "○ NO CHANGE"
    
    print(f"\n{context_name}:")
    print(f"  Log-odds: {result['mean_preference']:+.6f}")
...
# ============================================================================
print("\n" + "="*80)
print("FINAL ASSESSMENT: DOES RAG WORK AT GENERATION LAYER?")
print("="*80)

if rag_range > 1.0:
    print("\n✓ STRONG RAG EFFECT: Context dramatically shifts generation preferences")
    print("  RAG successfully steers the generation layer")
elif rag_range > 0.3:
    print("\n✓ MODERATE RAG EFFECT: Context measurably shifts generation preferences")
    print("  RAG provides meaningful steering of generation")
elif rag_range > 0.1:
    print("\n⚠️ WEAK RAG EFFECT: Context slightly shifts generation preferences")
    print("  RAG has limited impact on generation layer")
else:
    print("\n✗ NO RAG EFFECT: Context doesn't shift generation preferences")
    print("  Generation layer is as frozen as embeddings")

print("\n" + "="*80)

# ============================================================================
# PLOTTING RESULTS AS FIGURES (Added section)
# ============================================================================
print("\nGenerating plots...")

# Extract data for plotting
contexts = list(test_results.keys())
mean_prefs = [test_results[c]['mean_preference'] for c in contexts]
ci_lowers = [test_results[c]['ci_lower'] for c in contexts]
ci_uppers = [test_results[c]['ci_upper'] for c in contexts]
deltas = [mean_prefs[i] - baseline if contexts[i] != 'no_context' else 0 for i in range(len(contexts))]

# Plot 1: Log-Odds Preferences with Error Bars
fig, ax = plt.subplots(figsize=(12, 6))
ax.bar(contexts, mean_prefs, yerr=[np.abs(np.array(mean_prefs) - np.array(ci_lowers)), np.abs(np.array(ci_uppers) - np.array(mean_prefs))], capsize=5, color='skyblue')
ax.set_ylabel('Log-Odds Preference (Redemptive - Corrupted)')
ax.set_title('Generation Preferences by RAG Context')
ax.axhline(0, color='red', linestyle='--', label='Neutral')
ax.legend()
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
plt.savefig('preferences_bar.png')
plt.close()
print("Saved: preferences_bar.png (Log-odds preferences with CI error bars)")

# Plot 2: Deltas from Baseline
fig, ax = plt.subplots(figsize=(12, 6))
ax.bar(contexts, deltas, color=['green' if d > 0 else 'red' for d in deltas])
ax.set_ylabel('Delta from Baseline (No Context)')
ax.set_title('RAG Impact Deltas')
ax.axhline(0, color='black', linestyle='--')
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
plt.savefig('deltas_bar.png')
plt.close()
print("Saved: deltas_bar.png (Deltas from baseline, green=improvement, red=degradation)")

# Plot 3: Top Terms for No_Context (example; repeat for others if needed)
top_a = test_results['no_context']['top_A'][:10]
top_b = test_results['no_context']['top_B'][:10]
labels_a = [t[0] for t in top_a]
probs_a = [t[1] for t in top_a]
labels_b = [t[0] for t in top_b]
probs_b = [t[1] for t in top_b]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 8))
ax1.barh(labels_a[::-1], probs_a[::-1], color='green')  # Reverse for top at top
ax1.set_title('Top Redemptive Probs (No Context)')
ax1.set_xlabel('Probability')
ax2.barh(labels_b[::-1], probs_b[::-1], color='red')
ax2.set_title('Top Corrupted Probs (No Context)')
ax2.set_xlabel('Probability')
plt.tight_layout()
plt.savefig('top_terms_no_context.png')
plt.close()
print("Saved: top_terms_no_context.png (Horizontal bars for top 10 A/B terms in no_context)")

# Optional: Add more plots, e.g., for each context
# For example, loop over contexts to generate per-context top terms plots
for ctx in contexts:
    top_a = test_results[ctx]['top_A'][:10]
    top_b = test_results[ctx]['top_B'][:10]
    labels_a = [t[0] for t in top_a]
    probs_a = [t[1] for t in top_a]
    labels_b = [t[0] for t in top_b]
    probs_b = [t[1] for t in top_b]
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 8))
    ax1.barh(labels_a[::-1], probs_a[::-1], color='green')
    ax1.set_title(f'Top Redemptive Probs ({ctx})')
    ax1.set_xlabel('Probability')
    ax2.barh(labels_b[::-1], probs_b[::-1], color='red')
    ax2.set_title(f'Top Corrupted Probs ({ctx})')
    ax2.set_xlabel('Probability')
    plt.tight_layout()
    plt.savefig(f'top_terms_{ctx}.png')
    plt.close()
    print(f"Saved: top_terms_{ctx}.png")

print("\nAll plots generated! Open the PNG files to view figures.")
